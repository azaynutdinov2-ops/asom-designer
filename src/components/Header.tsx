...
import { Sparkles, Menu, X, ArrowUpRight, ShieldCheck, Flame } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = () => {
  ...
  const navLinks = [
    ...
    { name: "Dastur", href: "#modules" },
    { name: "Afzalliklar", href: "#why-us" },
    { name: "Mentor", href: "#mentor" },
    { name: "Bitiruvchilar Ishlari", href: "#gallery" },
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2 text-center sm:text-left">
          <div className="flex items-center justify-center gap-2 mx-auto sm:mx-0">
            <span className="bg-amber-400/20 text-amber-200 border border-amber-300/30 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
              <Flame className="w-3 h-3 fill-amber-300 text-amber-300" /> Yangi Guruh
            </span>
            <span>Chegirmali ro'yxatdan o'tish yopilishiga atigi <b>7 ta joy</b> qoldi!</span>
          </div>
          <a
          ...
          >
            Joyingizni band qiling <ArrowUpRight className="w-3.5 h-3.5" />
          </a>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <a href="#" className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-slate-300 font-medium">Grafik Dizayn & AI Kurslari</span>
              </div>
            </a>
            ...
            <nav className="hidden lg:flex items-center gap-6">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-sm text-slate-200 hover:text-white font-semibold transition-colors relative py-1 hover:after:w-full after:w-0 after:h-0.5 after:bg-purple-400 after:absolute after:bottom-0 after:left-0 after:transition-all after:duration-300"
                >
                  {link.name}
                </a>
              ))}
            ...
            <div className="flex lg:hidden items-center gap-3">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="sm:hidden text-xs font-semibold px-3 py-1.5 rounded-lg bg-purple-600 text-white shadow-md"
              >
                Ro'yxatdan o'tish
              </a>
              <button
          ...
          <div className="lg:hidden bg-[#0d1322] border-b border-slate-800 px-4 pt-4 pb-6 mt-3 space-y-3 animate-in fade-in slide-in-from-top duration-200">
            <div className="flex flex-col space-y-2">
              {navLinks.map((link) => (
                <a
                  ...
                  className="text-base text-slate-200 hover:text-purple-400 font-medium py-2 px-3 rounded-lg hover:bg-slate-800/50 transition-colors"
                >
                  {link.name}
                </a>
              ))}
            ...
            <div className="pt-3 border-t border-slate-800 flex flex-col gap-2">
              <a
                ...
                rel="noopener noreferrer"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg flex items-center justify-center gap-2"
              >
